...
Данный проект содержит 4 bash-скрипта, предназначенных для выполнения конкретных задач по обработке файловой системы преподователя предметов Криптозоология и Пивоварение, и 1 главный скрипт, предназначенный для запуска остальных скриптов.
...
|lab4_main.sh|Главный скрипт, который запускает остальные скрипты|
...
## bash_script_2.sh
Скрипт, который выводит студента выбранной группы с лучшей посещаемостью. После запуска скрипта пользователю предлагается ввести название группы и выбрать предмет (криптозология или пивоварение). Вывод содержит имя пользователя в БАРС и количество посещенных им занятий.
### Пример работы скрипта
...
## lab4_main.sh
Главный скрипт, который запускает остальные скрипты. После запуска пользователю предлагается ввести путь к каталогу labfiles или оставить путь по умолчанию (каталог labfiles находится в одной директории со скриптом). Далее появляется меню, в котором необходимо выбрать, какой скрипт требуется запустить. Кроме опций запуска 4-х вышеописанных скриптов, имеется опция вывода доступных файлов посещаемости групп. После выбора любой опции и завершении работы каждого скрипта, терминал возвращается в главное меню. Для выхода из меню и скрипта lab4_main.sh нужно выбрать соответствующую опцию.
### Пример работы скрипта
